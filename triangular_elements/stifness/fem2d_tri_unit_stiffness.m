addpath('../bc_integral')
addpath('../jacobian')
function k = fem2d_tri_unit_stiffness(x)
...
%           phi_{1, 2, 3} are in counter clockwise order
	...
	for iq = 1 : n_quadrature
		[sh, dtm] = fem2d_tri_basis_and_jacobian(qx(iq), qy(iq), x);
		d_N = sh(1 : 2, :);
	...
	if (( (x(1, 1) == 0) && (x(1, 2) == 0) ) ...  % on x == 0
	 ...
	 || ( (x(2, 1) == 1) && (x(2, 2) == 1) ))     % on y == 1
		k2 = k2 + fem2d_tri_integral_alpha(x, 1, 2);
	end
	...
	if (( (x(1, 2) == 0) && (x(1, 3) == 0) ) ...  % on x == 0
	 ...
	 || ( (x(2, 2) == 1) && (x(2, 3) == 1) ))     % on y == 1
		k2 = k2 + fem2d_tri_integral_alpha(x, 2, 3);
	end
	...
	if (( (x(1, 3) == 0) && (x(1, 1) == 0) ) ...  % on x == 0
	 ...
	 || ( (x(2, 3) == 1) && (x(2, 1) == 1) ))     % on y == 1
		k2 = k2 + fem2d_tri_integral_alpha(x, 3, 1);
	end
