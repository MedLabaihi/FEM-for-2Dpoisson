addpath('../jacobian')
addpath('../bc_integral')
function k = fem2d_quad_unit_stiffness(x)
...
%           phi_{1, 2, 3, 4} are in counter clockwise order
    ...
    for ix = 1 : n_quadrature
        for iy = 1 : n_quadrature
            [sh, dtm] = fem2d_quad_basis_and_jacobian(q(ix), q(iy), x, 0);
            d_N = sh(1 : 2, :);
    ...
    if (( (x(1, 1) == 0) && (x(1, 2) == 0) ) ...  % on x == 0
     ...
     || ( (x(2, 1) == 1) && (x(2, 2) == 1) ))     % on y == 1
        k2 = k2 + fem2d_quad_bc_alpha_integral(x, 1, 2);
    end
    ...
    if (( (x(1, 2) == 0) && (x(1, 3) == 0) ) ...  % on x == 0
     ...
     || ( (x(2, 2) == 1) && (x(2, 3) == 1) ))     % on y == 1
        k2 = k2 + fem2d_quad_bc_alpha_integral(x, 2, 3);
    end
    ...
    if (( (x(1, 3) == 0) && (x(1, 4) == 0) ) ...  % on x == 0
     ...
     || ( (x(2, 3) == 1) && (x(2, 4) == 1) ))     % on y == 1
        k2 = k2 + fem2d_quad_bc_alpha_integral(x, 3, 4);
    end
    ...
    if (( (x(1, 4) == 0) && (x(1, 1) == 0) ) ...  % on x == 0
     ...
     || ( (x(2, 4) == 1) && (x(2, 1) == 1) ))     % on y == 1
        k2 = k2 + fem2d_quad_bc_alpha_integral(x, 4, 1);
    end
